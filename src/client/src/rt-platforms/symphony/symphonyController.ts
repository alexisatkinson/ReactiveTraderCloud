...
import { SYMPHONY_APP_ID, createTileMessage, FX_ENTITY_TYPE, placeholderMessage } from './constants'
...
export const initiateSymphony = async (env?: string) => {
  ...
  entityController.implement({
    pause(id) {
      entityService.update(id, placeholderMessage, {
        symbol: map[id],
        version: VERSION,
        type: FX_ENTITY_TYPE,
      })
  ...
  const createNavItem = (id: string, title: string) =>
    navService.add(id, { title, icon: `${host}/symphony/logo.png` }, MENU_CONTROLLER)
